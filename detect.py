...
def fixation_detection(x, y, time, missing=0.0, maxdist=25, mindur=50):
	##，用于检测眼动数据中的注视点。它将一系列坐标点分成注视点和扫视点。
	# 注视点是连续的坐标点，它们之间的距离小于一定的像素距离。
	# 这个函数需要传入三个参数：x，y和time，分别代表x坐标，y坐标和时间戳。
	# 此外，还可以传入三个可选参数：missing，maxdist和mindur，分别代表缺失数据的值、最大距离和最小持续时间。
	# 函数返回两个列表：Sfix和Efix，分别代表注视点的开始时间和注视点的开始时间、结束时间、持续时间、结束x坐标和结束y坐标。
...
def saccade_detection(x, y, time, missing=0.0, minlen=5, maxvel=40, maxacc=340):
	...
	# of the squared horizontal and vertical interdistance
	diff_x = [x[i + 2] - x[i] for i in range(len(x) - 2)]
	diff_y = [y[i + 2] - y[i] for i in range(len(y) - 2)]
	diff_x = np.array(diff_x)
	diff_y = np.array(diff_y)
	intdist = ((diff_x**2 + diff_y**2)**0.5)*1000
	# get inter-sample times
	inttime = [time[i + 2] - time[i] for i in range(len(time) - 2)]
	inttime = np.array(inttime)
